...
MAX_SELF_REFINE_ITERATIONS: int = 2  # ! CONFIGURABLE
...
# * SAME-LEVEL SPIN PROMPT — used when spinning again at the same cascade level
SPIN_SAME_LEVEL_PROMPT: str = (
    "The previous attempt at this cascade level did not meet the quality threshold. "
    "The judge provided individual feedback for each answer below. "
    "Carefully review the judge's feedback and generate significantly improved answers that directly address the criticisms."
)
...
NUM_MAX_QUESTIONS: int = 750  # ! CONFIGURABLE
...
# SPINNING — retries at the same cascade level before escalating
MAX_LOOPS_PER_LEVEL: int = int(os.getenv("MAX_LOOPS_PER_LEVEL", "2"))
# Minimum score improvement required to justify another spin at the same level
...
EXPERIMENT_NAME: str = f"complex_workflow_spin_{NUM_MAX_QUESTIONS}"  # ! CONFIGURABLE
        ...
        try:
            ...
            with mlflow.start_run(run_name=question_id):
                ...
                for current_cascade_level in range(1, MAX_CASCADE_LEVEL + 1):
                    ...
                    # * STEP 3: Load models and create agents once per cascade level
                    ...
                    if not cascade_lvl_models:
                        logger.error(f"No models config found for cascade level {current_cascade_level}. Skipping.")
                        continue
                    ...
                    worker_agents_dict: dict[str, WorkingAgent] = initialize_worker_agents(
                        models_config=cascade_lvl_models, cascade_lvl=current_cascade_level)
                    ...
                    # ── SPIN LOOP ── retry at this cascade level before escalating
                    for spin_num in range(MAX_LOOPS_PER_LEVEL):
                        logger.info(f"Cascade level {current_cascade_level} — spin {spin_num + 1}/{MAX_LOOPS_PER_LEVEL}")

                        # * STEP 4: Generate initial responses → debate → refine
                        initial_answers, init_cost = asyncio.run(run_cascade_initial_answer(
                        ...
                        for agent_initial_answer in initial_answers.values():
                            if isinstance(agent_initial_answer, str):
                                continue
                        ...
                        for agent_critique in critiques.values():
                            if isinstance(agent_critique, str):
                                continue
                            print(f"\nAgent {agent_critique.author_id}: \n\tResponse: {agent_critique.content}")

                        final_answers, refine_cost = asyncio.run(run_cascade_refinement_loop(
                            worker_agents=worker_agents_dict,
                        ...
                        if not final_answers:
                            logger.error(f"No final answers at level {current_cascade_level} spin {spin_num + 1}. Skipping spin.")
                            break

                        cascade_total_cost += refine_cost
                        mlflow.log_metric(f"lvl{current_cascade_level}_spin{spin_num + 1}_cost", cascade_total_cost)
                        mlflow.log_metric("total_cost", cascade_total_cost)

                        for agent_final_answer in final_answers.values():
                            if isinstance(agent_final_answer, str):
                                continue
                            print(f"\nAgent {agent_final_answer.author_id}: \n\tResponse: {agent_final_answer.content}")

                        # * STEP 5: Judge evaluation
                        validator_prompt: str = ensemble_agents_answers(
                            agents_answers=final_answers,
                            initial_question=question_prompt,
                            premise_clause=JUDGE_PROMPT_3)
                        validator_answer: dict[str, Any] = asyncio.run(run_validation(
                            judge_agent=validator_agent,
                            question=question_prompt,
                        ...
                        if not validator_answer:
                            ...
                            validator_answer = {
                                "evaluation": {
                                    ...
                                    "individual_reviews": {
                                        first_valid: {"confidence_score": 0.0, "reason": "Judge unavailable"}
                                    }
                                }
                            }

                        current_score = validator_answer['evaluation']['best_answer']['best_confidence_score']
                        improvement = current_score - prev_score
                        mlflow.log_metric(f"lvl{current_cascade_level}_spin{spin_num + 1}_score", current_score)
                        mlflow.log_metric(f"lvl{current_cascade_level}_spin{spin_num + 1}_improvement", round(improvement, 4))

                        logger.info(f"Level {current_cascade_level} spin {spin_num + 1}: score={current_score:.4f}, improvement={improvement:+.4f}")

                        # ── SUCCESS ──
                        if current_score >= ACCEPTABLE_SCORE:
                            level_success = True
                            break

                        # ── STOP SPINNING: no meaningful improvement ──
                        if spin_num > 0 and improvement < IMPROVEMENT_THRESHOLD:
                            logger.info(
                                f"Improvement {improvement:+.4f} < threshold {IMPROVEMENT_THRESHOLD} "
                                f"at level {current_cascade_level} — escalating to next level")
                            break

                        prev_score = current_score

                        # ── KEEP SPINNING: feed judge's individual feedback back into the same level ──
                        spin_prompt: str = ensemble_agents_answers(
                            agents_answers=final_answers,
                            initial_question=original_question,
                            premise_clause=SPIN_SAME_LEVEL_PROMPT,
                            agents_answers_review=validator_answer)  # includes full per-worker judge feedback
                        prompt_data = (question_id, spin_prompt)
                        logger.info(f"Spinning again at level {current_cascade_level} with judge feedback.")
                    # ── END SPIN LOOP ──

                    if level_success:
                        # Log and save the successful answer
                        ...
                        synthetized_final_answer: str = asyncio.run(
                            synthetize_final_answer(validator_agent=validator_agent, final_answers=final_answers))
                        ...
                        log_final_answer_trace(
                            ...
                            is_definitive=True,
                            acceptable_score=ACCEPTABLE_SCORE,
                            synthetized_answer=final_answers[best_worker_id_clean].content,
                            best_worker_model_id=cascade_lvl_models[best_worker_id_clean]['model_name'],
                        ...
                        save_results_to_jsonl(
                            ...
                            answer=final_answers[best_worker_id_clean].content,
                            cascade_lvl=current_cascade_level,
                            winner_model=cascade_lvl_models[best_worker_id_clean]['model_name'],
                            judge_model=validator_agent.model_name,
                            score=current_score,
                            save_to_path_file=results_file)
                        break  # done — exit cascade level loop

                    # ── ESCALATE: build context prompt for next cascade level ──
                    logger.info(f"Level {current_cascade_level} exhausted. Best score: {current_score:.4f}. Escalating.")
                    log_final_answer_trace(
                        question_id=question_id,
                        question_prompt=original_question,
                        is_definitive=False,
                    ...
                    next_cascade_prompt: str = ensemble_agents_answers(
                        agents_answers=final_answers,
                        initial_question=original_question,
                ...
                # LAST LEVEL OF CASCADE WITH NO SUCCESS
                    ...
                    logger.warning(
                        f"All {MAX_CASCADE_LEVEL} cascade levels exhausted (each with up to {MAX_LOOPS_PER_LEVEL} spins). "
                        f"Best score: {current_score:.4f}. Falling back to best available answer.")
                    ...
                    log_final_answer_trace(
                        ...
                        is_definitive=True,
                        acceptable_score=ACCEPTABLE_SCORE,
                        synthetized_answer=final_answers[best_worker_id].content,
                        best_worker_model_id=cascade_lvl_models[best_worker_id]['model_name'],
                        cascade_level=current_cascade_level,
                        best_confidence_score=current_score,
                        total_cost=cascade_total_cost)

                    print(f"Synthetized final answer:\n{synthetized_final_answer}")
                    print(f"FINAL ANSWER (DIRECT FROM WORKER):\n{final_answers[best_worker_id].content}")
                    ...
                    save_results_to_jsonl(
                        ...
                        category=question_category,
                        question=original_question,
                        answer=final_answers[best_worker_id].content,
                        cascade_lvl=current_cascade_level,
                        winner_model=cascade_lvl_models[best_worker_id]['model_name'],
                        judge_model=validator_agent.model_name,
                        score=current_score,
                        save_to_path_file=results_file)
                    logger.success(f"Cascade completed at level {current_cascade_level}.")
        except Exception as e:  # ADD EXCEPTION HANDLER
            logger.error(f"Question {question_id} failed with error: {e}")
